
<template>
  <div class="flex flex-col w-100 space-y-5 bg-white rounded-3xl px-4 py-4">
    <div class="flex justify-start">
      <h1 class="text-lg font-bold">Введите информацию</h1>
    </div>
      <div class="flex flex-col">
        <div class="flex justify-between my-1">
          <div class="flex flex-col w-64">
            <div class="mb-2">Наименование</div>
            <input
              id="houseName"
              name="houseName"
              v-model="warehouseById.name"
              type="text"
              placeholder=" "
              required
              class="rounded-full border-1 py-2 px-4 "
            />
          </div>
          <div class="flex flex-col w-64">
            <div class="mb-2">Адрес</div>
            <input
              id="address"
              name="address"
              v-model="warehouseById.address"
              type="text"
              placeholder=" "
              required
              class="rounded-full border-1 py-2 px-4"
            />
          </div>
        </div>
        <div class="flex justify-between my-1">
          <div class="flex flex-col w-64">
            <div class="mb-2">Latitude</div>
            <input
              id="latitude"
              name="latitude"
              v-model="warehouseById.geom.coordinates[0]"
              type="text"
              placeholder=" "
              required
              class="rounded-full border-1 py-2 px-4 "
            />
          </div>
          <div class="flex flex-col w-64">
            <div class="mb-2">Longitude</div>
            <input
              id="id"
              name="id"
              v-model="warehouseById.geom.coordinates[1]"
              type="text"
              placeholder=" "
              required
              class="rounded-full border-1 py-2 px-4"
            />
          </div>
        </div>
        <div class="flex justify-start my-1">
          <div class="flex flex-col w-64">
            <div class="mb-2">ID склада в Вашей системе</div>
            <input
              id="id"
              name="id"
              type="text"
              v-model="warehouseById.client_db_id"
              placeholder=" "
              required
              class="rounded-full border-1 py-2 px-4 "
            />
          </div>
        </div>
        <div class="flex justify-end my-1">
          <dp-button
            text="спасти"
            type="button"
            @click="editWareHouse()"
            class="
              text-white
              bg-purple-450
              bg-secondary
              disabled:opacity-50
            "
          />
        </div>
      </div>
    <p class="text-red-500 mt-1 w-full min-h-6">{{ errorMessage }}</p>
  </div>
</template>

<script>
import DpButton from '@/components/buttons/DpButton.vue'
import LoginInput from '@/components/inputs/LoginInput.vue'
import { computed, onMounted, ref } from '@vue/runtime-core'
import { useStore } from 'vuex'
import { useRouter } from 'vue-router'

export default {
  components: { DpButton, LoginInput },
  props: {
    wareHousesId: { type: String, required: true },
  },

  setup(props) {
    const store = useStore()
    const router = useRouter()
    const successMessage = ref('')
    const errorMessage = ref('')
    const loading = ref(false)
    onMounted(async () => {
      try {
        loading.value = true
        await store.dispatch('warehouse/fetchWarehouse')
        loading.value = false
      } catch (err) {
        loading.value = false
        errorMessage.value = `Loading Failed: ${
          err.message ||
          err.response?.statusText ||
          err.statusText ||
          err.status ||
          err
        }`
      }
    })
    const warehouseById = computed(() =>
      store.getters['warehouse/getEditWarehouseById'](props.wareHousesId),
    )
    const editWareHouse = async () => {
      try {
        errorMessage.value = ''
        const payload = {
          id: props.wareHousesId,
          payload: {
            name: warehouseById.value.name,
            address: warehouseById.value.address,
            client_db_id: warehouseById.value.client_db_id,
            geom: {
                type: "Point",
                coordinates: [
                  parseFloat(warehouseById.value.geom.coordinates[0]),
                  parseFloat(warehouseById.value.geom.coordinates[1])
              ]
            },
          }
        }
        await store.dispatch('warehouse/editWareHouse', payload)
        router.push("/warehouses")

      } catch (err) {
        console.error(err)
        errorMessage.value = err?.message || err
      }
    }

    return {
      warehouseById,
      loading,
      editWareHouse,
      successMessage,
      errorMessage,
    }
  },
}
</script>
